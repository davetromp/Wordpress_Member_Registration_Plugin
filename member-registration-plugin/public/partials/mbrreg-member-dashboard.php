		...
		<div class="mbrreg-dashboard-actions">
			<?php if ($is_admin): ?>
				<a href="<?php echo esc_url(admin_url('admin.php?page=mbrreg-members')); ?>"
					class="mbrreg-button mbrreg-button-secondary">
	...
	<?php if (!empty($members)): ?>
		...
		<?php foreach ($members as $index => $member): ?>
			<div class="mbrreg-member-card" data-member-id="<?php echo esc_attr($member->id); ?>">
				...
				<form class="mbrreg-profile-form mbrreg-form" method="post"
					...
					<fieldset class="mbrreg-fieldset">
						...
						<div class="mbrreg-form-row">
							...
							<input type="text" id="mbrreg-first-name-<?php echo esc_attr($member->id); ?>" name="first_name"
								value="<?php echo esc_attr($member->first_name); ?>" <?php echo get_option('mbrreg_require_first_name', false) ? 'required' : ''; ?>>
						</div>
						...
						<div class="mbrreg-form-row">
							...
							<input type="text" id="mbrreg-last-name-<?php echo esc_attr($member->id); ?>" name="last_name"
								value="<?php echo esc_attr($member->last_name); ?>" <?php echo get_option('mbrreg_require_last_name', false) ? 'required' : ''; ?>>
						</div>
					</fieldset>
							...
							<?php foreach ($custom_fields as $field): ?>
								...
								<div class="mbrreg-form-row">
									<label for="mbrreg-custom-<?php echo esc_attr($field->id); ?>-<?php echo esc_attr($member->id); ?>">
										...
										<?php if ($is_readonly): ?>
											<span
												class="mbrreg-readonly-badge"><?php esc_html_e('(View only)', 'member-registration-plugin'); ?></span>
										<?php endif; ?>
									</label>
									<?php
									...
									echo $custom_fields_handler->render_field_input(
										...
										esc_html($value),
										array(
											'id_prefix' => 'mbrreg-custom-' . esc_html($member->id) . '-',
											'readonly' => esc_html($is_readonly),
										)
									);
									?>
								</div>
					...
					<div class="mbrreg-form-row mbrreg-form-actions">
						...
						<?php if ('active' === $member->status): ?>
							<button type="button" class="mbrreg-button mbrreg-button-danger mbrreg-deactivate-btn"
								data-member-id="<?php echo esc_attr($member->id); ?>">
		...
		<?php if ($allow_multiple): ?>
			<div class="mbrreg-add-member-section">
				...
				<div class="mbrreg-add-member-form-container" style="display: none;">
					...
					<p class="mbrreg-info">
						<?php esc_html_e('Add another family member to your account.', 'member-registration-plugin'); ?>
					</p>
