		...
		<div class="mbrreg-admin-columns">
				...
				<div class="mbrreg-admin-card">
					...
					<table class="form-table">
						<tr>
							<th scope="row">
								<label
									for="first_name"><?php esc_html_e('First Name', 'member-registration-plugin'); ?></label>
							</th>
							<td>
								<input type="text" name="first_name" id="first_name"
									value="<?php echo esc_attr($member->first_name); ?>" class="regular-text">
							</td>
						</tr>
						<tr>
							<th scope="row">
								<label
									for="last_name"><?php esc_html_e('Last Name', 'member-registration-plugin'); ?></label>
							</th>
							<td>
								<input type="text" name="last_name" id="last_name"
									value="<?php echo esc_attr($member->last_name); ?>" class="regular-text">
							</td>
						</tr>
						<tr>
							<th scope="row">
							...
							<td>
								<input type="email" name="email" id="email"
									value="<?php echo esc_attr($member->email); ?>" class="regular-text">
								<p class="description">
									<?php esc_html_e('Changing this will update the WordPress user email for the account owner.', 'member-registration-plugin'); ?>
								</p>
							</td>
						</tr>
					</table>
									...
									<th scope="row">
										<label for="custom_<?php echo esc_attr($field->id); ?>">
											...
											<?php if ($field->is_admin_only): ?>
												<span class="dashicons dashicons-lock" style="color: #d63638; font-size: 14px;"
													title="<?php esc_attr_e('Admin only', 'member-registration-plugin'); ?>"></span>
											<?php endif; ?>
										</label>
									</th>
									<td>
										...
										echo $custom_fields_handler->render_field_input(
											$field,
											esc_html($value),
										...
										<?php if ($field->is_admin_only): ?>
											<p class="description">
												<?php esc_html_e('This field can only be edited by administrators.', 'member-registration-plugin'); ?>
											</p>
										<?php endif; ?>
									</td>
								</tr>
			...
			<div class="mbrreg-admin-column">
					...
					<table class="form-table">
						...
						<tr>
							...
							<td>
								...
								<p class="description">
									<?php esc_html_e('Member admins can view and manage other members from the frontend.', 'member-registration-plugin'); ?>
								</p>
							</td>
						</tr>
					</table>
				...
				<div class="mbrreg-admin-card">
					...
					<table class="form-table">
						...
						<tr>
							...
							<td>
								<a
									href="<?php echo esc_url(admin_url('user-edit.php?user_id=' . $member->user_id)); ?>">
						...
						<tr>
							...
							<td><?php echo esc_html(mbrreg_format_date($member->created_at, get_option('date_format') . ' ' . get_option('time_format'))); ?>
							</td>
						</tr>
						<tr>
							<th scope="row"><?php esc_html_e('Last Updated', 'member-registration-plugin'); ?></th>
							<td><?php echo esc_html(mbrreg_format_date($member->updated_at, get_option('date_format') . ' ' . get_option('time_format'))); ?>
							</td>
						</tr>
					</table>
				</div>
				...
				<div class="mbrreg-admin-card">
					...
					<p>
						<button type="submit"
							class="button button-primary button-large"><?php esc_html_e('Update Member', 'member-registration-plugin'); ?></button>
						<span class="spinner"></span>
					</p>
					<p>
						<button type="button" class="button button-link-delete mbrreg-delete-member"
					...
					<?php if ('pending' === $member->status): ?>
						<p>
							<button type="button" class="button mbrreg-resend-activation"
								data-member-id="<?php echo esc_attr($member->id); ?>">
